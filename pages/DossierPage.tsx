...
export const DossierPage: React.FC<DossierPageProps> = ({ clinic, isAdmin, onBack, onUpdate }) => {
  ...
  const handleInterest = () => {
    const text = encodeURIComponent(`Olá tenho interesse na clinica ${clinic.nome}`);
            ...
            <div className="space-y-8">
              ...
              {!isEditing && (
                <button 
                  ...
                  className="w-full bg-[#2563eb] text-white py-6 rounded-3xl font-black text-lg hover:bg-[#1d4ed8] hover:scale-[1.02] transition-all shadow-2xl flex items-center justify-center space-x-3 active:scale-95"
                >
                  ...
                  <span>TENHO INTERESSE NESTA CLÍNICA</span>
                </button>
              )}
              {isEditing && (
